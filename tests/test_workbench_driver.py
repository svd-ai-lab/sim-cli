...
class TestDetect:
    ...
    def test_detect_unrelated_py(self, driver):
        assert driver.detect(FIXTURES / "pybamm_spm_good.py") is False
    ...
    def test_detect_comsol(self, driver):
        assert driver.detect(FIXTURES / "comsol_good.py") is False
...
class TestFallback:
    ...
    def test_run_file_falls_back_when_sdk_import_fails(self, driver, monkeypatch):
        ...
        # Fallback to RunWB2 should produce a RunResult (regardless of script outcome)
        result = driver.run_file(FIXTURES / "workbench_good.wbjn")
        assert result.solver == "workbench"
        assert isinstance(result.exit_code, int)  # RunWB2 was actually invoked
