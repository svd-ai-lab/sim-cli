...
class TestPyBaMMDetect:
    def test_detects_pybamm_script(self):
        ...
        assert driver.detect(FIXTURES / "pybamm_spm_good.py") is True
    ...
    def test_detects_bad_import_script(self):
        ...
        driver = PyBaMMLDriver()
        assert driver.detect(FIXTURES / "pybamm_bad_import.py") is False


class TestPyBaMMLint:
    def test_good_script_passes(self):
        driver = PyBaMMLDriver()
        result = driver.lint(FIXTURES / "pybamm_spm_good.py")
        assert result.ok is True
        assert not any(d.level == "error" for d in result.diagnostics)

    def test_missing_import_fails(self):
        driver = PyBaMMLDriver()
        result = driver.lint(FIXTURES / "pybamm_bad_import.py")
    ...
    def test_no_solve_warns(self):
        driver = PyBaMMLDriver()
        result = driver.lint(FIXTURES / "pybamm_no_solve.py")
...
class TestLintCLI:
    def test_exit_code_zero_good(self):
        runner = CliRunner()
        result = runner.invoke(main, ["lint", str(FIXTURES / "pybamm_spm_good.py")])
        assert result.exit_code == 0

    def test_exit_code_one_bad(self):
        runner = CliRunner()
        result = runner.invoke(main, ["lint", str(FIXTURES / "pybamm_bad_import.py")])
    ...
    def test_json_output(self):
        ...
        result = runner.invoke(
            main, ["--json", "lint", str(FIXTURES / "pybamm_spm_good.py")]
        )
        assert result.exit_code == 0
