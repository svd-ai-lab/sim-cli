...
sim serve --host 0.0.0.0          # FastAPI-Server auf :7600

# 2. Vom Agenten / Laptop / irgendwo im Netzwerk:
...
Das ist die volle Schleife: **starten → steuern → beobachten → abbauen** — der Ingenieur kann optional die Solver-GUI in Echtzeit beobachten.
...
|---|---|---|
| `sim serve` | HTTP-Server starten, eine Solver-Session halten | `ollama serve` |
...
| `sim inspect` | Live-Session-Status abfragen | `docker inspect` |
| `sim ps` | Aktive Session zeigen | `docker ps` |
...
| `sim check` | Solver / Driver-Verfügbarkeit prüfen | `docker info` |
| `sim lint` | Pre-flight Static-Check für ein Skript | `ruff check` |
| `sim logs` | Run-Historie durchstöbern | `docker logs` |

Umgebungsvariablen: `SIM_HOST`, `SIM_PORT` für den Client; `SIM_DIR` (Standard `.sim/`) für Run-Storage.
