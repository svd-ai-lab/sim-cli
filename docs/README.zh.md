...
新加 driver 很简单：在 `src/sim/drivers/<name>/driver.py` 实现 `DriverProtocol`，然后在 `drivers/__init__.py` 里注册即可。最小参考实现见 `pybamm/driver.py`，完整持久会话示例见 `fluent/`。
...
src/sim/
  ...
  drivers/
    fluent/        参考示例：持久会话 driver
                   (driver.py + runtime.py + queries.py)
    pybamm/        参考示例：最小一次性 driver
    …              更多 —— 每个已注册的后端一个文件夹
    __init__.py    DRIVERS 注册表 —— 新后端在这里注册
