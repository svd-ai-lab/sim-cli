...
内置覆盖范围横跨 CFD、多物理场、电子热分析、隐式与显式结构 FEA、前后处理、网格生成、具身 AI / GPU 物理、分子动力学、优化 / MDAO、电池建模、热物性、电力系统与射频仿真、以及离散事件建模。具体的求解器既可通过内置注册表抵达，也可通过外置插件包接入 —— 参考插件实现见 [`sim-plugin-ltspice`](https://github.com/svd-ai-lab/sim-plugin-ltspice)。
...
新加 driver 很简单：把 `DriverProtocol` 实现放进 tree 里，或者作为 out-of-tree 插件通过 `sim.drivers` entry-point group 注册。插件参考（driver + skill 一起打包）见 [`sim-plugin-ltspice`](https://github.com/svd-ai-lab/sim-plugin-ltspice)。
