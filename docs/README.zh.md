...
插件覆盖范围横跨 CFD、多物理场、电子热分析、隐式与显式结构 FEA、前后处理、网格生成、具身 AI / GPU 物理、分子动力学、优化 / MDAO、电池建模、热物性、电力系统与射频仿真、以及离散事件建模。参考插件实现见 [`sim-plugin-ltspice`](https://github.com/svd-ai-lab/sim-plugin-ltspice)。
