...
sim serve --host 0.0.0.0          # :7600 で FastAPI サーバー起動

# 2. エージェント / ノート PC / ネットワーク内のどこからでも:
...
これが完全なループ：**起動 → 駆動 → 観察 → 撤収** ── エンジニアは必要に応じてソルバー GUI をリアルタイムで監視できます。
...
| コマンド | 機能 | アナロジー |
|---|---|---|
| `sim serve` | HTTP サーバーを起動し、1 つのソルバーセッションを保持 | `ollama serve` |
